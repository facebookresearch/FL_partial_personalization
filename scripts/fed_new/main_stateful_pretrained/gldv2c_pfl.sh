...
common_args="${common_args} --client_scheduler const --use_pretrained_model --log_test_every_n_rounds 150 \
            --global_scheduler linear --server_optimizer adam --server_lr 5e-5 --client_lr 1e-4  \
...
do
for pfl_algo in "pfl_alternating"
do
for train_mode in "adapter"
do
    name="gldv2c_resnetgn_${train_mode}_${pfl_algo}_pretrained_stateful_seed${seed}"
