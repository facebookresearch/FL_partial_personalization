...
do
    ...
    name="so_${arch_size}_${train_mode}_${l2}_${pfl_algo}_pretrained_lr${client_lr}_slr${server_lr}_val"
...
do
    name="so_${arch_size}_${train_mode}_${hidden_dim}_${pfl_algo}_pretrained_lr${client_lr}_slr${server_lr}_val"
    task_params="--client_lr ${client_lr} --server_lr ${server_lr} --pfl_algo ${pfl_algo} --personalize_on_client ${train_mode} --adapter_hidden_dim ${hidden_dim} --logfilename ${logdir}/${name} --savedir ${savedir}/${name}"
    list_of_jobs+=("${task_params}")
done
