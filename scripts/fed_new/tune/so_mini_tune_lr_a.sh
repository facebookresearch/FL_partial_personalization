...
common_args="${common_args} --arch_size ${arch_size} \
        --server_optimizer adam --server_lr 5e-4 --client_scheduler const \
        --client_optimizer sgd --global_scheduler linear --global_warmup_fraction 0.1 \
        --pretrained_model_path /checkpoint/pillutla/pfl/saved_models2/so_mini_pretrain_val/checkpoint.pt \
        --validation_mode "

# Populate the array
list_of_jobs=()

for client_lr in 1 0.1
do
...
do
    ...
    name="so_${arch_size}_${train_mode}_${l2}_${pfl_algo}_pretrained_lr${client_lr}_slr5e-4_val"
    task_params="--client_lr ${client_lr} --pfl_algo ${pfl_algo} --personalize_on_client ${train_mode} --layers_to_finetune ${layers} --logfilename ${logdir}/${name} --savedir ${savedir}/${name}"
...
do
    name="so_${arch_size}_${train_mode}_${hidden_dim}_${pfl_algo}_pretrained_lr${client_lr}_slr5e-4_val"
    task_params="--client_lr ${client_lr} --pfl_algo ${pfl_algo} --personalize_on_client ${train_mode} --adapter_hidden_dim ${hidden_dim} --logfilename ${logdir}/${name} --savedir ${savedir}/${name}"
    list_of_jobs+=("${task_params}")
done
