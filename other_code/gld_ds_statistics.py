# run from this directory itself
# time python -u gld_ds_statistics.py
...
from load_gldv2 import load_data
...
train_data, test_data = load_data(cache_dir=data_dir)
...
t1 = time.time() ; train_lengths = get_lengths_from_client_ids(train_data); print('train:', time.time() - t1);

with open('../dataset_statistics/gldv2_all_sizes.p', 'wb') as f:
     pkl.dump([train_lengths], f)
